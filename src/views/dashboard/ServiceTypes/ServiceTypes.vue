...
import ViewServiceType from './ViewServiceTypes.vue';
import EditServiceType from './EditServiceTypes.vue';
import CreateServiceType from './CreateServiceTypes.vue';
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { title: 'Service Type (EN)', key: 'name', filterable: true,},
                { title: 'Service Type (AR)', key: 'name_ar', filterable: true,},
    ...
    methods: {
        async getData() {
          this.loading = true;
          try {
            const responseData = await fetchWrapper.post(`${base_url}/admin/service-types`, { 
        ...
        download() {
          const { token } = useAuthStore();
            this.loading = true;
            axios.post(`${base_url}/admin/service-types/export`, {
              from_date: this.dateRange ? this.formatDate(this.dateRange[0]) : null,
              to_date: this.dateRange ? this.formatDate(this.dateRange[1]) : null,
              search_keyword: this.search,
            }, {
            ...
            .then(response => {
                ...
                link.href = url;
                link.setAttribute('download', `service-types_${new Date().getTime()}.xlsx`);
        ...
        async deleteData(){
          this.loading = true;
          try {
            const responseData = await fetchWrapper.post(`${base_url}/admin/service-types/delete/${this.selectedRow.id}`, { });
      ...
      <v-card variant="flat">
        <v-card-item>
          <div class="d-sm-flex align-center justify-space-between">
            <v-card-title>Service Types</v-card-title>
            <div>
