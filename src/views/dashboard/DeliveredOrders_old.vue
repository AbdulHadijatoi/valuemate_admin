    ...
    data() {
        return {
            ...
            headers: [
                ...
                { title: 'Driver Name', key: 'driver_name', filterable: true, },
        ...
        <v-card-text>
          ...
          <v-data-table-server @update:options="getData" density="compact" v-model:page="page" :items-length="totalItems" v-model:items-per-page="perPage" :loading="loading" :headers="headers" :items="orders" class="elevation-0">
            ...
            <template v-slot:item.actions="{ item }">
              <v-chip color="success" rounded="pill" class="mr-2 py-5" @click="viewRequest(item.a_barcode)">
                <EyeIcon size="20"/>
              </v-chip>
            </template>
          </v-data-table-server>
