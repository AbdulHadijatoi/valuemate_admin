...
import { FilterIcon, LocationFilledIcon } from 'vue-tabler-icons';
...
export default {
    ...
    data() {
        ...
        return {
            ...
            orderHeaders: [
                { title: 'Branch Name', key: 'collection_branch_name', filterable: true,},
    ...
    methods: {
        async getCountsData() {
          this.loading = true;
          try {
            const responseData = await fetchWrapper.post(`${base_url}/collection-orders/counts`, { 
        ...
        async getData() {
          ...
          this.loading = true;
          try {
            const responseData = await fetchWrapper.post(`${base_url}/collection-orders`, { 
        ...
        downloadCountsData() {
          ...
          XLSX.writeFile(workbook, 'collection-orders-counts.xlsx');
        },
        ...
        openListDialog(item) {
          if(item.total_orders>0){
            ...
            this.selected_branch_id = item.branch_id;
            this.selected_branch_name = item.branch_name;
            this.data = [];
          }
        },
...
<template>
      ...
      <v-card variant="flat">
        ...
        <v-card-text>
          ...
          <v-row>
            ...
            <v-col cols="12" md="4">
              <v-btn color="primary" elevation="0" @click="getCountsData()"><FilterIcon size="20" class="mr-2"/>Apply Filter</v-btn>
            </v-col>
          </v-row>
  ...
  <v-dialog v-model="listDialog" max-width="90%" width="800px" min-height="400px" max-height="90%">
    <v-card>
      ...
      <v-card-text>
        <!-- <v-data-table-server @update:options="getData()" density="compact" v-model:page="page" :items-length="totalItems" v-model:items-per-page="perPage" :loading="loading" :headers="orderHeaders" :items="data" class="elevation-0"   :footer-props="{ itemsPerPageOptions: [10, 20, 50, 100, 200] }"> -->
          <v-data-table-server
