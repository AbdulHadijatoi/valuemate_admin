    ...
    methods: {
        filterData() {
          ...
          if (!this.filter.property_type_id) {
            errorMessage("Please select a property type to filter documents.");
            return;
          }

          this.data = this.allData.filter(item => {
            const matchesProperty = item.property_type_id == this.filter.property_type_id;
            const matchesService = this.filter.service_type_id
              ? item.service_type_id == this.filter.service_type_id
              : true;
        ...
        async getData() {
          ...
          try {
            ...
            if (responseData && responseData.data) {
              ...
              if(this.all_service_types.length == 0){
                this.all_service_types = responseData.service_types;
              }
